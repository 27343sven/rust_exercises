...
impl<'a> HighScores<'a> {
    pub fn new(scores: &'a[u32]) -> Self {
        let mut high_scores = scores.to_vec();
        high_scores.sort_by(|a, b| b.cmp(a));
        HighScores { scores, high_scores: high_scores.into_iter().take(3).collect::<Vec<_>>() }
    }
